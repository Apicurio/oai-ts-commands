...
describe("New Server (3.0)", () => {
    ...
    it("New Server (Document)", () => {
        commandTest(
            "tests/fixtures/new-server/3.0/new-server-document.before.json",
            "tests/fixtures/new-server/3.0/new-server-document.after.json",
    ...
    it("New Server (Document) [Already Exists]", () => {
        commandTest(
            "tests/fixtures/new-server/3.0/new-server-document-exists.before.json",
            "tests/fixtures/new-server/3.0/new-server-document-exists.after.json",
            (document: Oas30Document) => {
                let server: Oas30Server = document.createServer();
                server.url = "https://development.gigantic-server.com/v1";
                server.description = "Development server";
    ...
    it("New Server (Path Item)", () => {
        commandTest(
            "tests/fixtures/new-server/3.0/new-server-pathItem.before.json",
            "tests/fixtures/new-server/3.0/new-server-pathItem.after.json",
            (document: Oas30Document) => {
    ...
    it("New Server (Operation)", () => {
        commandTest(
            "tests/fixtures/new-server/3.0/new-server-operation.before.json",
            "tests/fixtures/new-server/3.0/new-server-operation.after.json",
            (document: Oas30Document) => {
